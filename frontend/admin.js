    ...
    try {
        ...
        livres.forEach(l => {
            if (l.source === 'api') {
                ...
                // Badge disponibilité
                const lienExterne = (l.lien || l.link || "").trim();
                const nbEmprunts = empruntsExternes.filter(e => (e.link === lienExterne || e.lien === lienExterne)).length;
                const dispoHtml = nbEmprunts > 0 ? `<span class="admin-card-status admin-card-status--busy">Emprunté ${nbEmprunts} fois</span>` : `<span class="admin-card-status">Disponible</span>`;
...
async function ajouterLivreDemo() {
    ...
    try {
        ...
        if (addRes.ok) {
            let payload = null;
            try { payload = await addRes.json(); } catch (e) { payload = null; }
            status.textContent = "ARCHIVAGE RÉUSSI !" + (payload && payload.id ? (` ID=${payload.id}`) : '');
